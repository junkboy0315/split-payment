...
export class AppPage {
  ...
  canCalculateCorrectly() {
    ...
    const isResultSameWith = (expectedValues: number[]) => {
      return new Promise((resolve) => {
        $$('li.list-group-item input').getAttribute('value')
    ...
    return $('input').sendKeys('15000')
      ...
      // delete person[2]
      .then(() => $$('.container-of-person-badge').get(2).click())
      .then(() => browser.sleep(500)) // wait for animation
      .then(() => isResultSameWith([ 5000, 2500, 3750, 3750 ]))

      // change person[3]'s payment to 4000
      .then(() => $$('.list-group-item input').get(3).sendKeys(
  ...
  canFixCorrectly() {
    ...
    const isResultSameWith = (expectedValues: boolean[]) => {
      return new Promise((resolve) => {
        $$('li.list-group-item .dollar').getAttribute('class')
          .then((classes) => {
    ...
    .then(() => $$('.container-of-person-list .dollar').get(3).click())
    .then(() => isResultSameWith([ true, true, true, false, false ]))
